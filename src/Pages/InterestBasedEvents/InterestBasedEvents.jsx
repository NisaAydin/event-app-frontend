...
function InterestBasedEvents() {
  ...
  useEffect(() => {
    async function fetchInterestBasedEvents() {
      try {
        const response = await axios.get("http://localhost:3000/event/user-interest-events", {
          withCredentials: true, // JWT veya oturum doğrulama için
        });
        setEvents(response.data);
        setError("");
      } catch (error) {
        console.error("İlgi alanlarına göre etkinlik önerileri alınırken hata oluştu: ", error.message);
